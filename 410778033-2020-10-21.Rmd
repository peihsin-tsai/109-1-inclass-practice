...
```{r ans12}
Game5Time <- c(date=NBA1920Final567$game5$Date,
...
```{r ans13}
G5TimeParsed <- mdy_hm(paste(NBA1920Final567$game5$Date,"2020", 
...
```{r ans15}
lubridate::minutes(G5TimeParsed) 
...
```{r ans21}
chNumbers <- factor(chNumbers, levels = c("壹","貳","參","肆","伍","陸","柒","捌","玖"))
table(chNumbers)
sumChNumbers <- sum(as.numeric(chNumbers))
```
